...
## 2026-07-20 — 7/20 POST-NIGHTLY AUDIT + transworld enrich-cap coverage fix (found via backfill verify)
7/20 nightly succeeded (started 15:02 UTC). Audit: source_health 30/30 green (backlog 36); source_quality total 22,864 active (T1 129 / T2 259), broker coverage 41% (+1), gaps none-actionable ✅, freshness all <48h; auto_promote caught-up (46/46 reviewed, 0 new = steady state).

**Transworld state-backfill (2fb66ab) verify → surfaced a bigger gap.** Backfill PARTIALLY landed: the Water-Management T2 row got state=OH ✓, but the HVAC T2 row was STILL null (T1/T2 null-state 2→1). Root cause: transworld has **642 listings clearing the $300K enrich floor but max_detail_enrich was 150**, and enrichDetails sliced targets in SCRAPE ORDER — so ~492 eligible listings (incl. high-value thesis-fit deals) got NO detail enrichment: no broker, no state, no EBITDA. Evidence: only **14 of 56 thesis-fit transworld rows had a broker linked** (25%); overall transworld broker% = 4%. The $1.8M HVAC deal was missed purely by ordering.
